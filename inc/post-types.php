...
function set_custom_cpt_columns($columns) {
    ...
    if($post_type=='restaurant') {
        unset( $columns['date'] );
        $columns['company_logo'] = __( 'Logo', 'acstarter' );
...
function custom_post_column( $column, $post_id ) {
    ...
    if($post_type=='restaurant') {
        switch ( $column ) {
            case 'company_logo' :
                $img = get_field('company_logo',$post_id);
                $img_src = ($img) ? $img['sizes']['thumbnail'] : '';
                $the_photo = '<span class="tmphoto" style="display:inline-block;width:50px;height:50px;background-color:#e2e1e1;text-align:center;">';
                if($img_src) {
